...
const FormQuestion = (props: PropTypes) => {
  ...
  const fetchDataQuestion = async () => {
    ...
    if (questionList) {
      ...
      const existingLocalId = questionList.map(
        (question: any) => question.local_id
      );
      //filtering local_id to get new local_id have not be created
      const newLocalId = Array.from(
        { length: questionLength },
        (_, index) => index + 1
      ).filter((index) => !existingLocalId.includes(index));
  ...
  const [questions, setQuestions] = useState<string[]>(
    Array.from({ length: indexForm.length }, () => "")
  );
  ...
  const [isOptions, setIsOptions] = useState<boolean[]>(
    Array.from({ length: indexForm.length }, () => true)
  );
            ...
            <>
              ...
              {indexForm.map((index) => (
                ...
                >
                  ...
                  >
                    <Form.Item>
                      <Upload {...imageProps} onChange={() => setIdxImg(index)}>
                    ...
                    >
                      {!isOptions[index] ? (
                        ...
                        >
                          This is{" "}
                          <span style={{ color: "blue" }}>Essay mode,</span>{" "}
                          Click to change to be{" "}
                          <span style={{ color: "blue" }}>Options mode</span>
                        </p>
                      ) : (
                        <p
                          style={{
                  ...
                  {isOptions[index] && (
                    <>
                      ...
                      >
                        <Input
                          onChange={(e) =>
                            handleOptionChange(index, 4, e.target.value)
                          }
                        />
                      </Form.Item>
                    </>
                  ...
                  <Form.Item
                    ...
                    label={`Answer ${index}`}
                    required={questions[index] ? true : false}
                  >
                    <Input
                      required={questions[index] ? true : false}
                      value={answers[index]}
